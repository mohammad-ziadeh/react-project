...
//     create: async (task) => {
//       try {
//         const response = await axios.post(API_URL, task, { headers: { 'Content-Type': 'application/json' } });
...
//     update: async (data) => {
//       try {
//         await axios.put(`${API_URL}/${data.id}`, data, { headers: { 'Content-Type': 'application/json' } });
...
//   return (
//     <div style={{ margin: 'auto', width: 'fit-content' }} key={refreshKey}>
//       {' '}
...
const Crud = () => {
  ...
  const reorderIds = async (updatedTasks) => {
    const reorderedTasks = updatedTasks.map((task, index) => ({ ...task, id: index + 1 }));
  ...
  const service = {
    ...
    delete: async (data) => {
      try {
        ...
        setTasks((prevTasks) => {
          const updatedTasks = prevTasks.filter((task) => task.id !== data.id);
          reorderIds(updatedTasks);
          return updatedTasks;
        });
